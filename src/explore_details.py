...
class DetailsData:
    ...
    def all_density_of_runs(self) -> pd.DataFrame:
        count_df = self.details_df.groupby(
            ['current_innings', 'runs']).size().reset_index(name='count')
        return count_df
    ...
    def inning_density_of_runs(self) -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        innings_1_count_df = innings_1_df.groupby(
            ['current_innings', 'runs']).size().reset_index(name='count')
        innings_2_count_df = innings_2_df.groupby(
            ['current_innings', 'runs']).size().reset_index(name='count')
